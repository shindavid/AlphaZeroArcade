...
class C4DataLoader:
    def __init__(self, args):
        ...
        filenames = list(natsorted(os.listdir(games_dir)))
        assert filenames, f'Empty dir: {games_dir}'
        extension = filenames[0].split('.')[-1]
        print('Loading data...')
        if extension == 'h5':
            # h5, generated by legacy python
            ...
            for filename in filenames:
                ...
                policy_output_data = data['policy']
                value_output_data = data['value']
                full_input_data.append(input_data)
                full_policy_output_data.append(policy_output_data)
                full_value_output_data.append(value_output_data)

            full_input_data = torch.concat(full_input_data)
            full_policy_output_data = torch.concat(full_policy_output_data)
            full_value_output_data = torch.concat(full_value_output_data)
        else:
            raise Exception(f'Unknown file extension: "{filenames[0]}" in {games_dir}')
...
def main():
    ...
    * momentum: KataGo used 0.9
    ...
    best_net.save_model(args.model_file, verbose=True)
