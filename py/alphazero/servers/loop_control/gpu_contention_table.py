...
DEFAULT_PRIORITIES: Dict[Domain, DomainState] = {
    ...
    Domain.SELF_EVAL: DomainState(DEFAULT_SELF_EVAL_PRIORITY),
    Domain.EVAL_VS_BENCHMARK: DomainState(DEFAULT_EVAL_PRIORITY),
...
class GpuContentionTable:
    ...
    def _pre_acquire_lock(self, domain: Domain):
        logger.debug('Pre-acquiring lock for %s', domain)
        assert self._lock.locked(), 'LockTable must be locked'
