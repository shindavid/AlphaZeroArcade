...
template<core::GameStateConcept GameState, core::TensorizorConcept<GameState> Tensorizor>
NNEvaluationService<GameState, Tensorizor>*
NNEvaluationService<GameState, Tensorizor>::create(const ManagerParams& manager_params) {
...
template<core::GameStateConcept GameState, core::TensorizorConcept<GameState> Tensorizor>
inline NNEvaluationService<GameState, Tensorizor>::NNEvaluationService(const ManagerParams& manager_params)
...
{
  auto input_shape = util::to_std_array<int64_t>(batch_size_limit_, eigen_util::to_int64_std_array_v<InputShape>);
  auto policy_shape = util::to_std_array<int64_t>(batch_size_limit_, eigen_util::to_int64_std_array_v<PolicyShape>);
  auto value_shape = util::to_std_array<int64_t>(batch_size_limit_, eigen_util::to_int64_std_array_v<ValueShape>);

  torch_input_gpu_ = torch::empty(input_shape, torch_util::to_dtype_v<dtype>).to(at::Device(manager_params.cuda_device));
  ...
  if (kEnableProfiling) {
    ...
    auto profiling_file_path = manager_params.profiling_dir() / util::create_string("%s.txt", name.c_str());
...
template<core::GameStateConcept GameState, core::TensorizorConcept<GameState> Tensorizor>
void NNEvaluationService<GameState, Tensorizor>::get_cache_stats(
...
NNEvaluationService<GameState, Tensorizor>::check_cache(const Request& request, const cache_key_t& cache_key) {
...
NNEvaluationService<GameState, Tensorizor>::get_eval(
    const Request& request, int reserve_index, std::unique_lock<std::mutex>& metadata_lock)
{
  const char* func = __func__;
