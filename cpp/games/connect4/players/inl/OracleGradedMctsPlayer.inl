...
inline void OracleGrader::mistake_tracker_t::dump(const std::string descr) const {
  printf(
      "OracleGrader %s count:%d net-t0:%.6f net-t1:%.6f mcts-t0:%.6f mcts-t1:%.6f baseline:%.6f\n",
      descr.c_str(), count, mistake_total_prior_t0 / count, mistake_total_prior_t1 / count,
      mistake_total_posterior_t0 / count, mistake_total_posterior_t1 / count,
      baseline_total / count);
}

inline void OracleGrader::mistake_tracker_t::update(float mistake_rate_prior_t0,
                                                    float mistake_rate_prior_t1,
                                                    float mistake_rate_posterior_t0,
                                                    float mistake_rate_posterior_t1,
                                                    float baseline) {
...
inline void OracleGrader::update(int score, int move_number, float mistake_rate_prior_t0,
                                 float mistake_rate_prior_t1, float mistake_rate_posterior_t0,
                                 float mistake_rate_posterior_t1, float baseline) {
  ...
  overall_tracker_.update(mistake_rate_prior_t0, mistake_rate_prior_t1, mistake_rate_posterior_t0,
                          mistake_rate_posterior_t1, baseline);
}
...
inline void OracleGradedMctsPlayer::receive_state_change(core::seat_index_t seat,
...
inline OracleGradedMctsPlayer::ActionResponse OracleGradedMctsPlayer::get_action_response(
  ...
  if (search_mode != kRawPolicy) {
    ...
    if (result.best_score >= 0) {  // winning or drawn position
      ...
      auto policy_prior =
          GameStateTypes::local_to_global(mcts_search_results->policy_prior, valid_actions);
      ...
      const PolicyArray& visit_counts =
          eigen_util::reinterpret_as_array(mcts_search_results->counts);
      auto visit_distr = visit_counts / visit_counts.sum();
      update_mistake_stats(result, policy_prior_array, visit_distr, valid_actions,
...
inline void OracleGradedMctsPlayer::update_mistake_stats(const PerfectOracle::QueryResult& result,
                                                         const PolicyArray& net_policy_t1,
                                                         const PolicyArray& posterior_policy_t1,
                                                         const ActionMask& valid_actions,
                                                         int move_number) const {
  PolicyArray net_policy_t0 = (net_policy_t1 == net_policy_t1.maxCoeff()).template cast<dtype>();
  PolicyArray posterior_policy_t0 =
      (posterior_policy_t1 == posterior_policy_t1.maxCoeff()).template cast<dtype>();
  ...
  float mistake_prior_t0_rate =
      mistake_prior_t0_den ? mistake_prior_t0_num / mistake_prior_t0_den : 0;
  float mistake_mcts_t0_rate =
      mistake_posterior_t0_den ? mistake_posterior_t0_num / mistake_posterior_t0_den : 0;
  float mistake_prior_t1_rate =
      mistake_prior_t1_den ? mistake_prior_t1_num / mistake_prior_t1_den : 0;
  float mistake_mcts_t1_rate =
      mistake_posterior_t1_den ? mistake_posterior_t1_num / mistake_posterior_t1_den : 0;
