...
struct LcZeroInputFrame {
  LcZeroInputFrame() = default;
  LcZeroInputFrame(const GameState&);
  bool operator==(const LcZeroInputFrame& other) const = default;
...
static_assert(sizeof(LcZeroInputFrame) == 48);
...
template <>
struct hash<a0achess::LcZeroInputFrame> {
  size_t operator()(const a0achess::LcZeroInputFrame& frame) const {
    return util::PODHash<a0achess::LcZeroInputFrame>{}(frame);
  }
};

}  // namespace std

#include "inline/games/chess/LcZeroInputFrame.inl"
