...
constexpr line_mask_t find_stable_edge_helper(line_mask_t curr_player_mask,
  ...
  for (int x = 0; (x < 8); ++x) {
    ...
    stable = find_stable_edge_helper(player_mask_after, opponent_mask_after, stable);
    if (!stable) return 0;
    ...
    stable = find_stable_edge_helper(player_mask_after2, opponent_mask_after2, stable);
    if (!stable) return 0;
  }
  return stable;
}
...
constexpr line_mask_t find_stable_edge(line_mask_t curr_player_mask, line_mask_t opponent_mask) {
   ...
   */
  return detail::find_stable_edge_helper(curr_player_mask, opponent_mask, curr_player_mask) |
         detail::find_stable_edge_helper(opponent_mask, curr_player_mask, opponent_mask);
}
