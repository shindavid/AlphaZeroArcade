...
template <>
struct std::hash<carcassonne::GameState> {
  std::size_t operator()(const carcassonne::GameState& state) const;
};
...
class GameState {
 public:
  static constexpr int kNumPlayers = carcassonne::kNumPlayers;
...
static_assert(core::GameStateConcept<carcassonne::GameState>);
...
template <>
struct serializer<carcassonne::GameState> {
  using type = DeterministicGameSerializer<carcassonne::GameState>;
};
...
template <>
struct SearchResultsDumper<carcassonne::GameState> {
  using LocalPolicyArray = carcassonne::GameState::LocalPolicyArray;
  using SearchResults = mcts::SearchResults<carcassonne::GameState>;
...
#include <inline/games/carcassonne/GameState.inl>
