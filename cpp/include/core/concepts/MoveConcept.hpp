...
/*
 * All Game::Move types M must satisfy core::concepts::Move<M, InfoSet>.
 *
 * Requirements:
 * - Default- and copy-constructible
 * - POD (standard-layout + trivially copyable)
 * - to_str() -> std::string
 * - static from_str(const InfoSet&, std::string_view) -> M
 */
template <class M, class InfoSet>
concept Move = std::is_default_constructible_v<M> && std::is_copy_constructible_v<M> &&
               ...
               requires(const M& a, const M& b, const InfoSet& s, std::string_view sv) {
