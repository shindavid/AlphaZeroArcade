...
concept GameRules =
  requires(const State& const_state, const StateHistory& const_history, State& state,
           StateHistory& history, group::element_t sym, core::seat_index_t last_active_seat,
           core::action_t last_action, GameResultsTensor& results, action_mode_t action_mode) {
    { GR::init_state(state) };
