...
inline void HumanTuiPlayer::prompt_for_piece(const FullState& state, const p_map_t& p_map,
                                             Piece& p) {
  if (p >= 0) return;
...
inline bool HumanTuiPlayer::prompt_for_orientation(const FullState& state, const p_map_t& p_map,
...
inline bool HumanTuiPlayer::prompt_for_root_location(const FullState& state, const p_map_t& p_map,
                                                     Piece& p, PieceOrientation& po,
                                                     Location& root_loc) {
  const po_map_t& po_map = p_map.at(p);
