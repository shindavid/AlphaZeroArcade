...
inline const ChessBoard ChessBoard::kStartposBoard(ChessBoard::kStartposFen);

inline const BitBoard ChessBoard::kPawnMask = 0x00FFFFFFFFFFFF00ULL;

inline void ChessBoard::Clear() { *this = ChessBoard(); }
inline void ChessBoard::Mirror() {
  our_pieces_.Mirror();
...
inline MoveList ChessBoard::GeneratePseudolegalMoves() const {
  MoveList result;
  result.reserve(60);
...
inline bool ChessBoard::IsValid() const {
...
inline bool ChessBoard::ApplyMove(Move move) {
...
inline bool ChessBoard::IsUnderAttack(Square square) const {
...
inline KingAttackInfo ChessBoard::GenerateKingAttackInfo() const {
...
inline bool ChessBoard::IsLegalMove(Move move,
...
inline MoveList ChessBoard::GenerateLegalMoves() const {
...
inline void ChessBoard::PutPiece(Square square, PieceType piece, bool is_theirs) {
...
inline void ChessBoard::SetFromFen(std::string_view fen, int* rule50_ply, int* moves) {
  Clear();
  ...
  auto complain = [&](std::string_view msg) {
    throw util::Exception("Bad fen string ({}): {}", std::string(msg), std::string(fen));
  };
...
inline bool ChessBoard::HasMatingMaterial() const {
...
inline std::string ChessBoard::DebugString() const {
  auto fen = BoardToFen(*this);
  std::replace(fen.begin(), fen.end(), ' ', '_');
  return "https://lc0.org/fen/" + fen;
}

inline Move ChessBoard::ParseMove(std::string_view move_str) const {
  auto complain = [&move_str](std::string_view reason) {
    throw util::Exception("Invalid move ({}): {}",  std::string(reason), std::string(move_str));
  };
...
namespace {
char GetPieceAt(const ChessBoard& board, Square square) {
  char c = '\0';
...
inline std::string BoardToFen(const ChessBoard& in_board) {
