...
inline void HumanTuiPlayer::receive_state_change(core::seat_index_t seat, const BaseState& state,
                                                 core::action_t action) {
  if (move_history_) move_history_->append(action);
  base_t::receive_state_change(seat, state, action);
}

inline core::action_t HumanTuiPlayer::prompt_for_action(const BaseState&, const ActionMask&) {
