...
void AlgorithmsBase<Traits, Derived>::Backpropagator::run(Node* node, Edge* edge,
  ...
  ValueArray tmp_last_child_Qgamma_snapshot = stats.Qgamma;
  ...
  Derived::update_stats(stats, pi_arr, last_child_Qgamma_snapshot_, last_child_W_snapshot_, node,
                        edge, lookup_table_);

  // LOG_INFO("Update result:");
  // LOG_INFO("Qgamma = {} -> {}", fmt::streamed(tmp_last_child_Qgamma_snapshot.transpose()),
  //          fmt::streamed(stats.Qgamma.transpose()));
  ...
  last_child_Qgamma_snapshot_ = tmp_last_child_Qgamma_snapshot;
  last_child_W_snapshot_ = tmp_last_child_W_snapshot;
  // LOG_INFO("Updating snapshots @{}:", __LINE__);
  // LOG_INFO("last_child_Qgamma_snapshot_ = {}",
  //          fmt::streamed(last_child_Qgamma_snapshot_.transpose()));
...
void AlgorithmsBase<Traits, Derived>::Backpropagator::update_edge_snapshots_helper(
  ...
  if (load_edge_snapshots_before_update) {
    last_child_Qgamma_snapshot_ = edge->child_Qgamma_snapshot;
  ...
  edge->child_Qgamma_snapshot = child_stats.Qgamma;
...
void AlgorithmsBase<Traits, Derived>::init_node_stats_from_terminal(Node* node) {
  ...
  stats.Qgamma = stats.Q;
  stats.Qgamma_min = stats.Q;
  stats.Qgamma_max = stats.Q;
  stats.W.fill(0.f);
...
void AlgorithmsBase<Traits, Derived>::undo_virtual_update(Node* node, Edge* edge) {
  throw util::CleanException("virtual updates not yet support in gamma0");
}
...
void AlgorithmsBase<Traits, Derived>::load_evaluations(SearchContext& context) {
  ...
  for (auto& item : context.eval_request.fresh_items()) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      edge->child_Qgamma_snapshot = edge->child_AV;
      edge->child_W_snapshot = AU.row(i);
    }

    RELEASE_ASSERT(stats.RN == 0, "RN={}", stats.RN);

    LocalActionValueArray child_Qgamma_arr(n, kNumPlayers);
    ...
    for (int i = 0; i < n; i++) {
      ...
      child_Qgamma_arr.row(i) = edge->child_Qgamma_snapshot;
    }

    update_QW_fields(stable_data, pi_arr, child_Qgamma_arr, child_W_arr, stats);
  }
}
...
void AlgorithmsBase<Traits, Derived>::to_results(const GeneralContext& general_context,
  ...
  results.win_rates = stats.Qgamma;
  results.value_prior = stable_data.R;
  results.action_mode = mode;

  results.min_win_rates = stats.Qgamma_min;
  results.max_win_rates = stats.Qgamma_max;
...
void AlgorithmsBase<Traits, Derived>::update_stats(NodeStats& stats, LocalPolicyArray& pi_arr,
  ...
  if (stable_data.is_chance_node) {
    throw util::Exception("chance nodes not yet supported in gamma0");
  } else {
    // read child stats and pi values into arrays to avoid repeated locking
    LocalActionValueArray child_Qgamma_arr(num_valid_actions, kNumPlayers);
    ...
    for (int i = 0; i < num_valid_actions; i++) {
      ...
      if (!child) {
        child_Qgamma_arr.row(i) = child_edge->child_Qgamma_snapshot;
        child_W_arr.row(i) = child_edge->child_W_snapshot;
      } else {
        const auto child_stats = child->stats_safe();  // make a copy

        child_Qgamma_arr.row(i) = child_stats.Qgamma;
    ...
    Derived::update_policy(pi_arr, node, edge, lookup_table, updated_edge_arr_index,
                           last_child_Qgamma_snapshot[seat], last_child_W_snapshot[seat],
                           child_Qgamma_arr.col(seat), child_W_arr.col(seat));
...
void AlgorithmsBase<Traits, Derived>::update_policy(LocalPolicyArray& pi_arr, const Node* node,
  ...
  double theta_old;
  double omega_sq_old;
  compute_theta_omega_sq(old_child_Qgamma, old_child_W, theta_old, omega_sq_old);

  LocalPolicyArrayDouble theta_arr(arr_size);
  LocalPolicyArrayDouble omega_sq_arr(arr_size);

  for (int i = 0; i < arr_size; i++) {
    compute_theta_omega_sq(child_Qgamma_arr[i], child_W_arr[i], theta_arr[i], omega_sq_arr[i]);
  }
  ...
  LocalPolicyArrayDouble gamma_arr(arr_size);
  ...
  auto debug_dump = [&](const std::string& msg) {
    ...
    LOG_INFO("old_child_Qgamma: {}", old_child_Qgamma);
  ...
  for (int i = 0; i < arr_size; i++) {
    ...
    double alpha = detail::alpha_from_thetas(theta_old, omega_sq_old, theta_new, omega_sq_new,
                                             theta_i, omega_sq_i);
    double gamma = pi_arr[i];

    alpha_arr[i] = alpha;
    gamma_arr[i] = gamma;
  }

  double gamma_sum = gamma_arr.sum();
  if (gamma_sum > 0.0) {
    gamma_arr *= (1.0 / gamma_sum);
    LocalPolicyArrayDouble log_pi_arr = pi_arr.template cast<double>().log();
    double adjustment = (gamma_arr * alpha_arr).sum();
...
template <search::concepts::Traits Traits, typename Derived>
void AlgorithmsBase<Traits, Derived>::compute_theta_omega_sq(double Qgamma, double W, double& theta,
                                                             double& omega_sq) {
...
void AlgorithmsBase<Traits, Derived>::update_QW_fields(const NodeStableData& stable_data,
  ...
  // Qgamma is computed as a weighted average of V (the prior value) and piQgamma_sum (the expected
  ...
  // U entirely, since U influences the weight given to V in the Qgamma calculation. But it's being
  // ignored for the uncertainty calculation itself.

  ValueArray old_Qgamma = stats.Qgamma;
  ...
  stats.Qgamma_min = stats.Qgamma;
  stats.Qgamma_max = stats.Qgamma;
  stats.W_max = stats.W;
  ...
  if (search::kEnableSearchDebug) {
    ...
    LOG_INFO("  child_Qgamma_arr:\n{}", fmt::streamed(child_Qgamma_arr.transpose()));
    LOG_INFO("  child_W_arr:\n{}", fmt::streamed(child_W_arr.transpose()));
    LOG_INFO("  pi: {}", fmt::streamed(pi_arr.transpose()));
    LOG_INFO("  piQgamma_sum: {}", fmt::streamed(piQgamma_sum.transpose()));
    LOG_INFO("  piQgamma_sq_sum: {}", fmt::streamed(piQgamma_sq_sum.transpose()));
    LOG_INFO("  piW_sum: {}", fmt::streamed(piW_sum.transpose()));
    LOG_INFO("  old Qgamma: {}", fmt::streamed(old_Qgamma.transpose()));
    LOG_INFO("  new Qgamma: {}", fmt::streamed(stats.Qgamma.transpose()));
