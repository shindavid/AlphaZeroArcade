...
TEST(MetaProgramming, DispatchType) {
  ...
  bool found = mp::dispatch_type<L>(
      []<typename T>(std::type_identity<T>) { return std::is_same_v<T, float>; },
  ...
  int call_count = 0;
  found = mp::dispatch_type<L>(
      []<typename T>(std::type_identity<T>) { return std::is_arithmetic_v<T>; },
