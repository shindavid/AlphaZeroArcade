...
using C4ValueArray = C4Game::Traits::ValueArray;
...
TEST(C4GameLogRoundTrip, SerializeDeserialize) {
  ...
  std::vector<int> target_indices = {0, 1, 2, 3, 4, 5, 6, 7, 8};
  ...
  for (int row = 0; row < sg.num_samples; ++row) {
    ...
    for (int i = 0; i < ChildStatsTensor::Dimensions::total_size; ++i) {
      EXPECT_FLOAT_EQ(output[offset + i], 0.0f) << "child_stats mismatch at row=" << row;
    }
    offset += ChildStatsTensor::Dimensions::total_size;
    EXPECT_FLOAT_EQ(output[offset], 0.0f);
    offset++;
