...
bool extractModelConfig(const std::string& onnxFile, ModelConfig& config) {
  ...
  for (int i = 0; i < network->getNbInputs(); ++i) {
    ...
    config.inputs.push_back(
      {t->getName(), t->getDimensions(), getVolumePerBatch(t->getDimensions())});
  }
...
PerfStats timeInferenceV3(IExecutionContext* context, cudaStream_t stream,
                          const ModelConfig& config, const std::vector<void*>& d_inputs,
                          const std::vector<void*>& d_outputs, int actualBatch, int warmupRuns = 10,
                          int numRuns = 100) {
...
void printTable(const std::string& title, const std::vector<PerfStats>& results, int minBatch,
...
bool parseArgs(int argc, char** argv, Args& args) {
  ...
  desc.add_options()("help", "show help message")("model", po::value<std::string>(&args.model_path),
                                                  "path to model .onnx")(
    "precision", po::value<std::string>(&args.precision)->default_value(args.precision),
    "Build precision: fp32, fp16, or int8")(
    "min-batch", po::value<int>(&args.min_batch)->default_value(args.min_batch),
    "min batch size in optimization profile")(
    "max-batch", po::value<int>(&args.max_batch)->default_value(args.max_batch),
    "max batch size in optimization profile")(
    "opt-batch", po::value<int>(&args.opt_batch)->default_value(args.opt_batch),
    "opt batch size in optimization profile")(
  ...
  if (args.precision != "fp32" && args.precision != "fp16" && args.precision != "int8") {
    std::cerr << "Invalid precision '" << args.precision
              << "'. Allowed values: fp32, fp16, int8.\n\n";
    std::cerr << desc << std::endl;
    return false;
  }
...
int main(int argc, char** argv) {
  ...
  for (size_t i = 0; i < config.inputs.size(); ++i) {
    CHECK_CUDA(
      cudaMalloc(&d_inputs[i], args.max_batch * config.inputs[i].volPerBatch * sizeof(float)));
  }

  std::vector<void*> d_outputs(config.outputs.size(), nullptr);
  for (size_t i = 0; i < config.outputs.size(); ++i) {
    CHECK_CUDA(
      cudaMalloc(&d_outputs[i], args.max_batch * config.outputs[i].volPerBatch * sizeof(float)));
  }
  ...
  if (engine) {
    ...
    for (size_t j = 0; j < args.test_batch_sizes.size(); ++j) {
      ...
      results[j] = timeInferenceV3(context, stream, config, d_inputs, d_outputs, actualBatch,
                                   args.warmup_runs, args.num_runs);
    }
